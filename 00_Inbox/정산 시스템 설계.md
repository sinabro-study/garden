# 정산 시스템 설계 (System Design)
- 기준 문서: [[정산 시스템 구축 가이드]], [[정산 시스템 역할별 할 일]]
- 관점: 백엔드 엔지니어 — 전체 아키텍처, 배치 파이프라인, 데이터 모델, 지급 처리 설계

## 1. 설계 목표와 전제
| 항목 | 내용 |
| --- | --- |
| 최우선 가치 | 정합성 > 실시간성 — 확정 금액은 배치로 산출, 실시간 값은 예상치로만 제공 |
| 처리 방식 | 배치 기반 확정 + 이벤트 기반 원천 수집 (하이브리드) |
| 정합성 장치 | 원천 불변(Append-only), 요율 스냅샷, 복식부기 원장, 다단계 대사 |
| 멱등성 | 배치 재실행·이체 요청 모두 멱등 보장 (재실행 시 결과 동일) |
| 격리 | 정산 DB를 운영(주문/결제) DB와 분리 — 배치 부하·장애 격리 |
| 추적성 | 모든 금액은 원천 거래까지 역추적 가능해야 함 |

## 2. 전체 아키텍처
```mermaid
flowchart LR
    subgraph CO[커머스 서비스]
        ORD[주문·결제 서비스]
        OBX[(Outbox)]
    end
    subgraph EV[이벤트 파이프라인]
        RLY[Outbox 릴레이]
        KF[(Kafka)]
    end
    subgraph ST[정산 시스템]
        COL[원천 수집기]
        SDB[(정산 DB)]
        BAT[정산 배치]
        LED[(원장 Ledger)]
        REC[대사 모듈]
        PAYM[지급 모듈]
        ADM[정산 API · 어드민]
    end
    subgraph EX[외부 시스템]
        PG[PG사 EDI/API]
        BK[펌뱅킹]
    end
    subgraph DT[데이터 팀]
        DW[(DW · BI)]
    end
    ORD ==>|로컬 트랜잭션 내 기록| OBX
    OBX ==>|CDC 읽기| RLY
    RLY ==>|발행| KF
    KF ==>|소비| COL
    COL ==>|원천 적재| SDB
    SDB ==> BAT
    BAT ==>|계산·집계 저장| SDB
    BAT ==>|분개 기록| LED
    SDB ==> REC
    PG -.->|거래내역 수신| REC
    REC ==>|대사 통과 → 확정| PAYM
    PAYM ==>|이체 요청| BK
    BK -.->|결과 통보·조회| PAYM
    SDB ==> ADM
    SDB -.->|CDC| DW
    DW -.->|크로스체크| REC
    linkStyle 9,12,14,15 stroke:#ef6c00,stroke-width:2px

    classDef core fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef store fill:#e8f5e9,stroke:#2e7d32,color:#1b5e20
    classDef ext fill:#fff3e0,stroke:#ef6c00,color:#e65100
    class ORD,RLY,COL,BAT,REC,PAYM,ADM core
    class OBX,KF,SDB,LED,DW store
    class PG,BK ext
```
- 다이어그램 범례
  - 굵은 검정 실선: 내부 동기 흐름
  - 주황 점선: 외부 연동·비동기 흐름
  - 파랑: 애플리케이션 컴포넌트
  - 초록: 저장소
  - 주황: 외부 시스템
- 핵심 설계 포인트
  - Outbox 패턴: 결제 데이터 변경과 Outbox INSERT를 같은 로컬 트랜잭션으로 커밋 — "결제는 됐는데 이벤트는 유실" 상황을 원천 차단
  - 이벤트 경로: Outbox → 릴레이(CDC)가 읽어 Kafka 발행 → 원천 수집기는 Kafka만 소비 (커머스 DB 직접 조회 금지 — 결합 방지)
  - 원천 수집기: 릴레이가 at-least-once이므로 중복 발행 가능 — 원천 유니크 키 기반 멱등 처리로 중복 차단
  - 정산 DB 분리: 배치 대량 조회가 운영 트래픽에 영향 주지 않도록 물리 분리
  - DW 크로스체크: 데이터 팀 독립 집계와 상호 대사하여 이중 검증 ([[정산 시스템 역할별 할 일]] 4.2 참조)

## 3. 컴포넌트별 책임
| 컴포넌트 | 책임 | 비고 |
| --- | --- | --- |
| Outbox 릴레이 | Outbox 테이블 변경분을 Kafka로 발행 | Debezium CDC, at-least-once |
| 원천 수집기 | Kafka 이벤트 소비, SettlementSource 불변 적재 | Outbox 직접 조회 금지, 멱등 처리 |
| 정산 배치 | 대상 선별 → 건별 계산 → 집계 → 검증 | Spring Batch, 회차 단위 실행 |
| 대사 모듈 | PG 거래내역 수신·파싱, 자동 매칭, 불일치 큐 관리 | 확정의 선행 조건 |
| 지급 모듈 | 이체 실행, 멱등키 관리, 실패 재처리 | 펌뱅킹 연동 |
| 원장 | 복식부기 분개 기록 (Append-only) | 잔액 대사의 기준 |
| 정산 API · 어드민 | 파트너 조회, 운영 처리, 조정 승인 워크플로 | 권한 분리 (Maker-Checker) |

## 4. 정산 배치 파이프라인
```mermaid
flowchart LR
    S1[대상 선별] ==> S2[건별 계산] ==> S3[회차 집계] ==> S4[합계 검증] ==> S5[대사] ==> S6[확정] ==> S7[지급 실행]
    S4 -. 불일치 .-> E1[HOLD · 수동 확인]
    S5 -. 불일치 .-> E1
    E1 -. 해소 후 재실행 .-> S2
    linkStyle 6,7,8 stroke:#ef6c00,stroke-width:2px

    classDef normal fill:#e3f2fd,stroke:#1565c0,color:#0d47a1
    classDef final fill:#e8f5e9,stroke:#2e7d32,color:#1b5e20
    classDef warn fill:#fff3e0,stroke:#ef6c00,color:#e65100
    class S1,S2,S3,S4,S5,S6 normal
    class S7 final
    class E1 warn
```
- 다이어그램 범례
  - 굵은 검정 실선: 정상 흐름
  - 주황 점선: 예외 흐름 (검증·대사 불일치 시 HOLD 후 해소 시점부터 재실행)
- 단계별 설계 원칙
  - 대상 선별: 마감 시각(Cut-off) 기준 스냅샷 — 이후 유입 원천은 다음 회차로 귀속
  - 건별 계산: 요율은 거래 시점 스냅샷 사용, 계산 근거를 SettlementDetail에 함께 저장
  - 집계·검증: 건별 합 = 집계값 검증을 배치 단계로 내장, 불일치 시 확정 차단
  - 멱등성: 회차 단위 재실행 시 기존 산출물 삭제 후 재생성, 단계별 체크포인트로 부분 재실행 지원
  - 이중 실행 방지: 분산 락으로 동일 회차 동시 실행 차단

## 5. 데이터 모델 (ERD)
```mermaid
erDiagram
    SETTLEMENT_TARGET ||--o{ SETTLEMENT : "회차 생성"
    SETTLEMENT ||--o{ SETTLEMENT_DETAIL : "집계"
    SETTLEMENT_SOURCE ||--o| SETTLEMENT_DETAIL : "계산 근거"
    FEE_POLICY ||--o{ SETTLEMENT_DETAIL : "요율 스냅샷"
    SETTLEMENT ||--o{ PAYOUT : "지급 실행"
    SETTLEMENT ||--o{ ADJUSTMENT : "조정 반영"
    SETTLEMENT ||--o{ LEDGER_ENTRY : "분개"
    SETTLEMENT_SOURCE {
        bigint id PK
        string order_id
        string tx_type "PAYMENT/CANCEL/REFUND"
        decimal amount
        date settlement_base_date
        datetime occurred_at
    }
    SETTLEMENT {
        bigint id PK
        bigint target_id FK
        date period_from
        date period_to
        decimal confirmed_amount
        string status
    }
    PAYOUT {
        bigint id PK
        bigint settlement_id FK
        string idempotency_key UK
        decimal amount
        string bank_result_code
        string status
    }
```
- 설계 원칙
  - SettlementSource: 불변 — 취소/환불은 마이너스 원천 레코드로 추가, UPDATE 금지
  - SettlementDetail: 적용 요율·계산 입력값 스냅샷 보관 (역추적 근거)
  - Payout: 이체 요청당 멱등키 유니크 제약으로 중복 이체 원천 차단
  - LedgerEntry: 차변/대변 쌍 기록, Append-only — 임의 시점 잔액 산출 가능

## 6. 지급 처리 시퀀스
```mermaid
sequenceDiagram
    participant B as 정산 배치
    participant P as 지급 모듈
    participant K as 펌뱅킹
    participant L as 원장
    B->>P: 지급 요청 (회차 ID, 멱등키)
    P->>P: 멱등키 중복 검사
    P->>K: 이체 실행 (멱등키 포함)
    alt 정상 응답
        K-->>P: 이체 성공
        P->>L: 분개 기록 (지급 완료)
        P-->>B: PAID 전이
    else 타임아웃 · 응답 유실
        P->>K: 이체 결과 조회 API 호출
        K-->>P: 최종 상태 회신
        P-->>B: 조회 결과 기준 상태 반영
    end
```
- 설계 원칙
  - 응답 유실 시 임의 재시도 금지 — 반드시 결과 조회로 최종 상태 확인 후 처리 (중복 이체 방지)
  - 부분 실패: 실패 건만 PAY_FAILED로 격리, 성공 건 지급은 유지
  - 이체 한도 초과 시 분할 이체 정책 적용

## 7. 기술 스택
| 영역     | 선택                                       | 근거                       |
| ------ | ---------------------------------------- | ------------------------ |
| 애플리케이션 | Spring Boot, Spring Batch                | 팀 주력 스택, 청크 처리·재시작 지원    |
| 정산 DB  | MySQL (운영 DB와 분리)                        | 트랜잭션 정합성, 금액은 DECIMAL 타입 |
| 이벤트    | Kafka + Outbox Relay (Debezium 등)        | 결제 이벤트 유실 방지, 순서 보장      |
| 분산 락   | ShedLock + JDBC(정산 DB) 락 저장소            | 배치와 실패 모드 일치 — 장애 지점 최소화 |
| 스케줄러   | Spring Batch + 스케줄링 도구 (Jenkins/Airflow) | 재실행·모니터링 용이성             |
| 모니터링   | 배치 실패·대사 불일치·지급 실패율 알림                   | Slack 알림 연동              |

## 8. 모듈 구성
| 모듈 | 역할 |
| --- | --- |
| settlement-domain | 엔티티, 계산 로직, 정책 (외부 의존성 없음 — 단위 테스트 집중 영역) |
| settlement-batch | 정산 배치 Job/Step 구성 |
| settlement-api | 파트너·어드민 API |
| settlement-worker | 이벤트 소비 (원천 수집기), 지급 실행 |
| settlement-infra | DB, Kafka, 뱅킹·PG 연동 어댑터 |
- 원칙: 수수료·공제 계산 로직은 settlement-domain 단일 소유 — 배치/API 어디서든 동일 로직 사용

## 9. 확장성·장애 대응
### 9.1 장애 시나리오별 대응
| 시나리오 | 대응 설계 |
| --- | --- |
| 거래량 증가로 배치 지연 | 정산 대상 기준 파티셔닝 병렬 처리 (Spring Batch Partitioning) |
| 배치 중 실패 | 단계별 체크포인트에서 재시작, 회차 단위 멱등 재실행 |
| 락 만료로 배치 이중 진입 | 다층 방어로 차단 (9.2 참조) — 락은 최적화, 정확성은 DB 상태가 보장 |
| 락 저장소 장애 | 락 저장소를 정산 DB(JDBC)로 통일하여 실패 모드 일치 — 이 경우 결과는 중복 실행이 아닌 미실행이며, 미실행은 모니터링(10)으로 감지 |
| 이벤트 유실 의심 | 일 단위 원천 건수 대사 (주문 DB count vs 원천 count) 배치로 탐지 |
| PG 파일 미수신 | 대사 단계 진입 차단, 수신 지연 알림 후 수동 트리거 |
| 뱅킹 장애 | 지급만 보류(CONFIRMED 유지), 복구 후 재개 — 확정과 지급의 분리 설계 덕분에 가능 |
| 정산 DB 장애 | 원천은 Kafka에 보존되므로 복구 후 재소비로 재구축 가능 |

### 9.2 배치 이중 실행 방어 계층
| 계층 | 장치 | 동작 |
| --- | --- | --- |
| 1차 | ShedLock (JDBC 락) | 정상 상황의 중복 실행 사전 차단 (best-effort, `lockAtMostFor` 만료 시 뚫릴 수 있음) |
| 2차 | 회차 상태 CAS | 조건부 UPDATE로 상태 선점 — 동시 진입 시 한쪽만 성공, 실패 측 즉시 중단 |
| 3차 | 산출물 유니크 제약 | (회차 ID, 원천 ID) 유니크 제약으로 중복 INSERT를 DB에서 차단 |
| 최종 | 지급 멱등키 | Payout 멱등키 유니크 제약 — 계산이 겹쳐도 이체는 두 번 실행 불가 |
- 2차 상태 CAS 예시: `UPDATE settlement SET status = 'AGGREGATING' WHERE id = ? AND status = 'CREATED'` — 갱신 0건이면 "이미 실행 중"으로 판정하고 종료
- Spring Batch JobRepository도 동일 JobInstance 동시 실행을 차단하므로 부가 방어 계층으로 동작
- 설계 사상: 락이 뚫려도 금전 정확성은 훼손되지 않아야 함 — 정확성 책임은 항상 DB 제약과 상태 머신에 둘 것

## 10. 모니터링
| 지표 | 감지 목적 | 알림 기준 예시 |
| --- | --- | --- |
| 배치 미실행 감지 (Dead Man's Switch) | 락 저장소 장애 등으로 조용히 실행되지 않는 상황 | 예정 시각 + 유예 시간 내 시작 이벤트 없으면 즉시 알림 |
| 배치 실패 | Job/Step 단위 실패 | 실패 즉시 알림 |
| 배치 처리 시간 | 지연 추세 감지 (파티셔닝 필요 시점 판단) | 평시 대비 급증 시 알림 |
| Kafka 컨슈머 랙 | 원천 수집 지연 — 마감 시점 원천 누락 위험 | 랙 임계치 초과 시 알림 |
| Outbox 릴레이 지연 | 이벤트 발행 적체 | 미발행 Outbox 행 체류 시간 초과 시 알림 |
| 대사 불일치 건수·금액 | 정합성 이상 조기 발견 | 임계치 초과 시 알림 |
| 지급 실패율 | 뱅킹 연동·계좌 오류 급증 | N% 초과 시 알림 |
| 정산 금액 급변동 | 계산 로직 오류·이상 거래 | 전 회차 대비 ±N% 이상 변동 시 확인 |
| HOLD 체류 시간 | 수동 처리 적체 | N일 초과 시 에스컬레이션 |
- 알림 채널: Slack 연동, 심각도 구분 (즉시 대응 / 익일 확인)
- 원칙: 마감·확정·지급 등 돈이 움직이는 시점의 이상은 실패 알림뿐 아니라 "해야 할 일이 일어나지 않음"도 감지 대상에 포함할 것
- 운영 지표 상세 기준은 [[정산 시스템 구축 가이드]] 12장 참조
